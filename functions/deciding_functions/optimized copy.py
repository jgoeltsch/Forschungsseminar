def validate_optimization_with_forecast_and_real(
    ...
    """
    Führt zwei Optimierungen durch:
    1. Optimierung mit Realdaten ("perfekte Information")
    2. Optimierung mit Vorhersagedaten (Forecast)
    Die Steuerungsvorgaben aus beiden Optimierungen werden jeweils auf die Realdaten angewendet.
    Gibt beide Ergebnis-DataFrames und die wichtigsten Abweichungen zurück.
    """
    # 1. Optimierung mit Realdaten (beides real)
    result_real = optimize_energy_flow(
        df_real,
        battery_capacity,
        initial_battery,
        charging_rate,
    ...
    # 2. Optimierung mit Forecastdaten, aber Auswertung auf Realdaten
    from .optimized_valid import optimize_energy_flow_valid
    result_forecast = optimize_energy_flow_valid(
        df_forecast,
        df_real,
        battery_capacity,
        initial_battery,
    ...
    # Vergleich: z.B. Gesamtkosten, SOC, Flüsse etc.
    # Hier beispielhaft: Gesamtkosten (auf Basis realer Spotpreise)
    total_cost_real = (result_real['spotprice_EUR_per_MWh'] * (result_real['grid_to_load_MWh'] + result_real['grid_to_batt_MWh'])
                      - export_price_factor * result_real['spotprice_EUR_per_MWh'] * result_real['ee_export_MWh']).sum()
    total_cost_forecast = (result_forecast['spotprice_EUR_per_MWh'] * (result_forecast['grid_to_load_MWh'] + result_forecast['grid_to_batt_MWh'])
                           - export_price_factor * result_forecast['spotprice_EUR_per_MWh'] * result_forecast['ee_export_MWh']).sum()
    abs_error_cost = abs(total_cost_real - total_cost_forecast)
    rel_error_cost = abs_error_cost / abs(total_cost_real) if total_cost_real != 0 else np.nan

    # Rückgabe: Ergebnisse und Fehlermaße
    return {
        'result_real': result_real,
        'result_forecast': result_forecast,
        'total_cost_real': total_cost_real,
        'total_cost_forecast': total_cost_forecast,
        'abs_error_cost': abs_error_cost,
        'rel_error_cost': rel_error_cost
    }
import pandas as pd
import numpy as np
