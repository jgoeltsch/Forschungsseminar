...
def calculate_kpis(df: pd.DataFrame, export_price_factor: float):
    grid_energy   = float((df["grid_to_load_MWh"] + df["grid_to_batt_MWh"]).sum())
    grid_cost     = float(((df["grid_to_load_MWh"] + df["grid_to_batt_MWh"]) * df["spotprice_EUR_per_MWh"]).sum())
    export_energy = float(df["ee_export_MWh"].sum())
    export_rev    = float((export_price_factor * df["ee_export_MWh"] * df["spotprice_EUR_per_MWh"]).sum())
